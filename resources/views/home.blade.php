...
/* Combine "Other" label and input neatly */
.other-field {
    display: flex;
    align-items: center;
    gap: 10px; /* Space between radio button/label and input */
}

/* Hide the input field initially */
.other-field .input-text {
    display: none;
}

/* When "Other" radio is selected, show input field */
.other-field input[type="radio"]:checked ~ .input-text {
    display: inline-block;
}
    </style>
</head>
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Function to handle each fieldset's "Other" radio functionality
            const initializeOtherFunctionality = (radioName, inputId) => {
                ...
                // Function to handle radio selection changes
                ...
                // Update the "Other" radio button value dynamically when typing
                otherInput.addEventListener('input', function () {
                    otherRadioButton.value = this.value;
                });

                // Add event listeners for all radio buttons in this group
                allRadioButtons.forEach((radio) => {
                    radio.addEventListener('change', handleRadioChange);
                });
            };

            // Initialize "Other" functionality for all forms
